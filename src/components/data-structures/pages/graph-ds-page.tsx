...
import type { GraphVariant } from '@/components/visualizers/data-structures/graph-ds'
...
export default function GraphDSPageContent() {
        ...
        <div className="flex flex-col gap-4">
          <Card>
            <CardHeader>
              ...
              {displayStep.message && (
                <p className="text-sm text-muted-foreground">{displayStep.message}</p>
              )}
              {displayStep.subMessage && (
                <p className="text-xs text-muted-foreground/70">{displayStep.subMessage}</p>
              )}
            </CardHeader>
            <CardContent>
              <div className="bg-muted rounded-lg overflow-hidden" style={{ height: 340 }}>
                <GraphDSCanvas
                  graph={graphData}
