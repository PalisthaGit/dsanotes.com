...
import type { HashTableType } from '@/components/visualizers/data-structures/hash-table'
import { useState } from 'react'

const TABLE_TYPES: { value: HashTableType; label: string; description: string }[] = [
  {
    value: 'linear',
    label: 'Linear Probing',
    description: 'On collision, scan forward one slot at a time: h(k, i) = (h(k) + i) mod m',
  },
  {
    value: 'quadratic',
    label: 'Quadratic Probing',
    description: 'On collision, probe with increasing squares: h(k, i) = (h(k) + i²) mod m',
  },
  {
    value: 'double',
    label: 'Double Hashing',
    description: 'On collision, use a second hash function: h(k, i) = (h₁(k) + i·h₂(k)) mod m',
  },
  {
    value: 'chaining',
    label: 'Separate Chaining',
    description: 'Each bucket holds a linked list. Multiple keys can share a bucket.',
  },
]

export default function HashTablePageContent() {
    ...
    <div className="flex flex-col gap-6 p-6 max-w-6xl mx-auto w-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6 lg:items-stretch">
        <div className="flex flex-col gap-6">
          ...
          <HashTableVisualizer step={displayStep} message={displayStep.message} />
        </div>
